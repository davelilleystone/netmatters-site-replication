<div class="contact-form-wrapper">
    <form class="contact-form" action="contact.php" method="post" novalidate>
        ...
        <input type="text" id="name" name="name" value="<?= $formInputs['name'] ?? '' ?>"
            <?= array_key_exists('name', $formErrors) ? $errorClassName : '' ?> required>
        ...
        <textarea name="message" id="message" cols="30" rows="10" value="<?= $formInputs['message'] ?? '' ?>"
            class="<?= array_key_exists('message', $formErrors) ? $errorClassName : '' ?>" required></textarea>
