<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="./css/slick.css" />
    <link rel="stylesheet" type="text/css" href="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick-theme.css" />
    <link rel="stylesheet" href="./css/normalize.css" />
    <link rel="stylesheet" href="./css/style.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css" />
    <script src="./js/sticky-nav.js" defer></script>
    <script src="./js/mobile-nav.js" defer></script>
    <script src="./js/cookie-consent.js" defer></script>
    <title>Full Service Digital Agency | Cambridgeshire & Norfolk | Netmatters</title>
</head>

<body>

    <div class="main-container">

        <!-- Cookie Consent -->

        <?php include './includes/cookie_consent.php'; ?>

        <!-- Sidebar -->
        <?php include './includes/sidebar.php'; ?>

        <!-- Sticky Nav -->
        <?php include './includes/sticky_nav.php'; ?>

        <div class="mobile-nav-overlay"></div>
        <div class="main-content">

            <!-- Standard Nav -->
            <?php include './includes/standard_nav.php'; ?>

            <!-- Hero -->
            <div class="slick-container">
                <!-- <div class="hero__overlay"></div> -->
                <div class="full-width-container full-width-container--hero hero--netmatters-overview slick-item">
                    <div class="hero__overlay"></div>
                    <div class="hero__content-container">
                        <div class="hero__content inner-container">
                            <h1 class="hero__heading">The East Of England's Leading Technology Company</h1>
                            <p class="hero__text">
                                Performance-driven digital and technology services <br />
                                with complete transparency.
                            </p>
                            <a href="#" class="hero__btn">why choose us? <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                        </div>
                    </div>
                </div>
                <div class="full-width-container full-width-container--hero hero--bespoke-software slick-item">
                    <div class="hero__overlay"></div>
                    <div class="hero__content-container">
                        <div class="hero__content inner-container">
                            <h1 class="hero__heading">Bespoke Software</h1>
                            <p class="hero__text">
                                Delivering expert bespoke software<br />
                                solutions across a range of industries.
                            </p>
                            <a href="#" class="hero__btn">find out more <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                        </div>
                    </div>
                </div>
                <div class="full-width-container full-width-container--hero hero--it-support slick-item">
                    <div class="hero__content-container">
                        <div class="hero__overlay"></div>
                        <div class="hero__content inner-container">
                            <h1 class="hero__heading">IT Support</h1>
                            <p class="hero__text">
                                Fast and cost-effective IT support<br />
                                services for your business.
                            </p>
                            <a href="#" class="hero__btn">find out more <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                        </div>
                    </div>
                </div>
                <div class="full-width-container full-width-container--hero hero--digital-marketing slick-item">
                    <div class="hero__overlay"></div>
                    <div class="hero__content-container">
                        <div class="hero__content inner-container">
                            <h1 class="hero__heading">Digital Marketing</h1>
                            <p class="hero__text">
                                Generating your new business through<br />
                                results-driven marketing activities.
                            </p>
                            <a href="#" class="hero__btn">find out more <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                        </div>
                    </div>
                </div>
                <div class="full-width-container full-width-container--hero hero--telecoms-services slick-item">
                    <div class="hero__overlay"></div>
                    <div class="hero__content-container">
                        <div class="hero__content inner-container">
                            <h1 class="hero__heading">Telecoms Services</h1>
                            <p class="hero__text">
                                A new approach to connectivity, see<br />
                                how we can help your business.
                            </p>
                            <a href="#" class="hero__btn">find out more <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                        </div>
                    </div>
                </div>
                <div class="full-width-container full-width-container--hero hero--web-design slick-item">
                    <div class="hero__overlay"></div>
                    <div class="hero__content-container">
                        <div class="hero__content inner-container">
                            <h1 class="hero__heading">Web Design</h1>
                            <p class="hero__text">
                                For businesses looking to make a strong <br />
                                and effective first impression.
                            </p>
                            <a href="#" class="hero__btn">find out more <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                        </div>
                    </div>
                </div>
                <div class="full-width-container full-width-container--hero hero--cyber-security slick-item">
                    <div class="hero__overlay"></div>
                    <div class="hero__content-container">
                        <div class="hero__content inner-container">
                            <h1 class="hero__heading">Cyber Security</h1>
                            <p class="hero__text">
                                Keeping businesses and their customers<br />
                                sensitive information protected
                            </p>
                            <a href="#" class="hero__btn">find out more <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Services -->

            <div class="full-width-container full-width-container--services">
                <div class="inner-container services-header">
                    <h2>Our Services</h2>
                    <a href="#">View Our Work</a>
                </div>
                <div class="inner-container services">
                    <div class="services__card services__card--bespoke-software">
                        <a class="services__card-overlay" href="#"></a>
                        <div class="services__card-inner">
                            <div class="services__card-inner-heading">
                                <span
                                    class="services__card-inner-heading-container services__card-icon icon-apps im-fs-32"></span>
                            </div>
                            <h2 class="services__card-inner-title">Bespoke Software</h2>
                            <p class="services__card-inner-text">
                                Bespoke software solutions for all your business needs including integrations and
                                reporting.
                            </p>
                            <a href="#" class="services__card-inner-btn">Read More</a>
                        </div>
                    </div>
                    <div class="services__card services__card--it-support">
                        <a class="services__card-overlay" href="#"></a>
                        <div class="services__card-inner">
                            <div class="services__card-inner-heading">
                                <span
                                    class="services__card-inner-heading-container services__card-icon icon-display im-fs-32"></span>
                            </div>
                            <h2 class="services__card-inner-title">IT Support</h2>
                            <p class="services__card-inner-text">
                                Fully managed IT support and consultancy packages tailored to meet your exact
                                business
                                needs.
                            </p>
                            <a href="#" class="services__card-inner-btn">Read More</a>
                        </div>
                    </div>
                    <div class="services__card services__card--digital-marketing">
                        <a class="services__card-overlay" href="#"></a>
                        <div class="services__card-inner">
                            <div class="services__card-inner-heading">
                                <span
                                    class="services__card-inner-heading-container services__card-icon icon-bar-graph im-fs-32"></span>
                            </div>
                            <h2 class="services__card-inner-title">Digital Marketing</h2>
                            <p class="services__card-inner-text">
                                Driven brand awareness & ROI through creative, tailored digital marketing campaigns.
                            </p>
                            <a href="#" class="services__card-inner-btn">Read More</a>
                        </div>
                    </div>
                    <div class="services__card services__card--telecoms-services">
                        <a class="services__card-overlay" href="#"></a>
                        <div class="services__card-inner">
                            <div class="services__card-inner-heading">
                                <span
                                    class="services__card-inner-heading-container services__card-icon icon-phone_in_talk im-fs-32"></span>
                            </div>
                            <h2 class="services__card-inner-title">Telecoms Services</h2>
                            <p class="services__card-inner-text">
                                Business telephony solutions including mobile & connectivity solutions.
                            </p>
                            <a href="#" class="services__card-inner-btn">Read More</a>
                        </div>
                    </div>
                    <div class="services__card services__card--web-design">
                        <a class="services__card-overlay" href="#"></a>
                        <div class="services__card-inner">
                            <div class="services__card-inner-heading">
                                <span
                                    class="services__card-inner-heading-container services__card-icon icon-code im-fs-32"></span>
                            </div>
                            <h2 class="services__card-inner-title">Web Design</h2>
                            <p class="services__card-inner-text">
                                User-centric design for businesses looking to make a lasting impression.
                            </p>
                            <a href="#" class="services__card-inner-btn">Read More</a>
                        </div>
                    </div>
                    <div class="services__card services__card--cyber-security">
                        <a class="services__card-overlay" href="#"></a>
                        <div class="services__card-inner">
                            <div class="services__card-inner-heading">
                                <span
                                    class="services__card-inner-heading-container services__card-icon icon-security im-fs-32"></span>
                            </div>
                            <h2 class="services__card-inner-title">Cyber Security</h2>
                            <p class="services__card-inner-text">Prevention, testing, consultancy & breach
                                management
                                services.</p>
                            <a href="#" class="services__card-inner-btn">Read More</a>
                        </div>
                    </div>
                    <div class="services__card services__card--developer-training">
                        <a class="services__card-overlay" href="#"></a>
                        <div class="services__card-inner">
                            <div class="services__card-inner-heading">
                                <span class="services__card-inner-heading-container"><i
                                        class="fa-solid fa-graduation-cap fa-2xl services__card-icon"></i></span>
                            </div>
                            <h2 class="services__card-inner-title">Developer Training</h2>
                            <p class="services__card-inner-text">
                                Web design &amp; software training courses designed to secure a job in tech.
                            </p>
                            <a href="#" class="services__card-inner-btn">Read More</a>
                        </div>
                    </div>
                </div>
                <div class="inner-container services-footer">
                    <a href="#">View Our Work</a>
                </div>
            </div>

            <!-- Partners -->

            <div class="full-width-container full-width-container--partners">
                <ul class="partners">
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/google-partner.webp"
                                alt="netmatters partner" />
                        </picture>
                    </li>
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/living_wage.webp"
                                alt="netmatters partner" />
                        </picture>
                    </li>
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/norfolk_prohelp.webp"
                                alt="netmatters partner" />
                        </picture>
                    </li>
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/future-50.webp"
                                alt="netmatters partner" />
                        </picture>
                    </li>
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/norfolk-carbon-charter.webp"
                                alt="netmatters partner" /></picture>
                    </li>
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/PPC_logo.webp"
                                alt="netmatters partner" />
                        </picture>
                    </li>
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/princess-royal-training.webp"
                                alt="netmatters partner" /></picture>
                    </li>
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/future-50.webp"
                                alt="netmatters partner" />
                        </picture>
                    </li>
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/qms-bw.webp"
                                alt="netmatters partner" /></picture>
                    </li>
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/iso_27001.webp"
                                alt="netmatters partner" />
                        </picture>
                    </li>
                    <li class="partners__item">
                        <picture><img class="partners__image" src="./img/partners/skills-of-tomorrow.webp"
                                alt="netmatters partner" /></picture>
                    </li>
                </ul>
            </div>

            <!-- Welcome -->

            <div class="full-width-container full-width-container--bg-welcome">
                <div class="inner-container welcome">
                    <div class="welcome__content-wrapper">
                        <h2 class="welcome__heading">welcome to netmatters</h2>
                        <p class="welcome__text welcome__text--semi-bold">
                            Netmatters is a leading Bespoke Software, IT Support, and Digital Marketing company
                            based in
                            the East of
                            England with offices in <a href="#" class="welcome__text-link">London</a>,
                            <a href="#" class="welcome__text-link">Cambridge</a>,
                            <a href="#" class="welcome__text-link">Wymondham</a>, and
                            <a href="#" class="welcome__text-link">Great Yarmouth</a>.
                        </p>
                        <p class="welcome__text">
                            We aren't tied into contracts with third-party providers, so you know that our
                            recommendations for your
                            business are based purely with one benefit in mind: to help improve your business with
                            the
                            most
                            appropriate solutions.
                        </p>
                        <p class="welcome__text">
                            We pride ourselves on being an ethical business and have a unique business offering and
                            cost
                            model that
                            ensures you get the most from our relationship in an upfront manner.
                        </p>
                        <div class="welcome__btn-container">
                            <a href="#" class="welcome__button welcome__button--thunder">why choose us? <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                            <a href="#" class="welcome__button welcome__button--thunder">our culture <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                        </div>
                    </div>

                    <div class="welcome__content-wrapper">
                        <h2 class="welcome__heading">what our clients think</h2>
                        <div class="welcome__ratings">
                            <span><i class="fa-solid fa-star"></i></span>
                            <span><i class="fa-solid fa-star"></i></span>
                            <span><i class="fa-solid fa-star"></i></span>
                            <span><i class="fa-solid fa-star"></i></span>
                            <span><i class="fa-solid fa-star"></i></span>
                        </div>
                        <q class="welcome__quote">Netmatters stood out from the start. Great guys and very easy to
                            work
                            with. Both
                            the build and digital
                            marketing teams are clearly skilled -they know their stuff! They delivered a website to
                            our
                            (high!)
                            expectations and went over and above to ensure we were satisfied clients - and we
                            are!</q>
                        <div class="welcome__quote-details">
                            <span class="welcome__quote-author">Eleanor Bishop, Head of Marketing</span><a href="#"
                                class="welcome__text-link"> - Ashcroft Partnership LLP</a>
                        </div>
                        <div class="welcome__btn-container">
                            <a href="#" class="welcome__button welcome__button--google">google reviews <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                            <a href="#" class="welcome__button welcome__button--tp">trustpilot reviews <span><i
                                        class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Latest News -->
            <div class="full-width-container full-width-container--latest-news">
                <div class="inner-container latest-news-header">
                    <h2>Latest News</h2>
                    <a href="#">View All</a>
                </div>
                <div class="inner-container latest-news">
                    <div class="latest-news__card latest-news__card--blue-theme">
                        <a class="latest-news__card-overlay" href="#"></a>
                        <header>
                            <picture>
                                <img class="latest-news__card-image" src="./img/latest-news/item-1-img.png"
                                    alt="congratulations adam" />
                            </picture>
                            <a class="latest-news__card-tooltip" href="#">news</a>
                        </header>
                        <div class="latest-news__card-content">
                            <h3 class="latest-news__card-content-heading">
                                <a href="#">Adam Greer Becomes the Netmatters Operations...</a>
                            </h3>

                            <p class="latest-news__card-content-text">
                                We are thrilled to announce the promotion of Adam Greer to Operations Director in
                                addition to his
                                cu...
                            </p>
                            <a href="#" class="latest-news__card-content-btn">Read More</a>
                            <footer>
                                <div class="latest-news__card-user">
                                    <div class="latest-news__card-user-icon">
                                        <picture><img src="./img/users/tessa-woodrow.jpg" alt="tessa dempsey" />
                                        </picture>
                                    </div>
                                    <div class="latest-news__card-user-meta-container">
                                        <div class="latest-news__card-user-posted-by">Posted by Tessa Dempsey</div>
                                        <div class="latest-news__card-user-date-posted">21st August 2023</div>
                                    </div>
                                </div>
                            </footer>
                        </div>
                    </div>

                    <!-- end card -->

                    <div class="latest-news__card latest-news__card--blue-theme">
                        <a class="latest-news__card-overlay" href="#"></a>
                        <header>
                            <picture>
                                <img class="latest-news__card-image" src="./img/latest-news/item-2-img.jpg"
                                    alt="now hiring" />
                            </picture>
                            <a class="latest-news__card-tooltip" href="#">careers</a>
                        </header>
                        <div class="latest-news__card-content">
                            <h3 class="latest-news__card-content-heading">
                                <a href="#">IT Office Administrator</a>
                            </h3>
                            <p class="latest-news__card-content-text">
                                Salary Range £21,675 - £25,000 + Bonuses + Pension Hours 40 hours per week, Monday –
                                Friday
                                Location...
                            </p>
                            <a href="#" class="latest-news__card-content-btn">Read More</a>
                            <footer>
                                <div class="latest-news__card-user">
                                    <div class="latest-news__card-user-icon">
                                        <picture><img src="./img/users/bethany-shakespeare.webp"
                                                alt="bethany shakespeare" /></picture>
                                    </div>
                                    <div class="latest-news__card-user-meta-container">
                                        <div class="latest-news__card-user-posted-by">Posted by Bethany Shakespeare
                                        </div>
                                        <div class="latest-news__card-user-date-posted">14th August 2023</div>
                                    </div>
                                </div>
                            </footer>
                        </div>
                    </div>

                    <!-- end card -->

                    <div class="latest-news__card latest-news__card--grey-theme latest-news__card--hide">
                        <a class="latest-news__card-overlay" href="#"></a>
                        <header>
                            <picture>
                                <img class="latest-news__card-image" src="./img/latest-news/item-3-img.png"
                                    alt="case study" />
                            </picture>
                            <a class="latest-news__card-tooltip" href="#">case studies</a>
                        </header>
                        <div class="latest-news__card-content">
                            <h3 class="latest-news__card-content-heading">
                                <a href="#">Spicerhaart – Upskilling Staff Through the Sk... <span>- 4 MINUTE
                                        READ</span></a>
                            </h3>
                            <p class="latest-news__card-content-text">
                                The Client Spicerhaart is an independent residential sales and lettings
                                organisation,
                                recognised as...
                            </p>
                            <a href="#" class="latest-news__card-content-btn">Read More</a>
                            <footer>
                                <div class="latest-news__card-user">
                                    <div class="latest-news__card-user-icon">
                                        <picture><img src="./img/users/netmatters.webp" alt="tessa dempsey" />
                                        </picture>
                                    </div>
                                    <div class="latest-news__card-user-meta-container">
                                        <div class="latest-news__card-user-posted-by">Posted by Netmatters</div>
                                        <div class="latest-news__card-user-date-posted">9th August 2023</div>
                                    </div>
                                </div>
                            </footer>
                        </div>
                    </div>
                    <!-- end card -->
                </div>
                <div class="inner-container latest-news__bottom-nav">
                    <a href="#">View All</a>
                </div>
            </div>

            <!-- Clients -->

            <div class="full-width-container full-width-container--clients">
                <ul class="clients">
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/busseys_logo.webp"
                                alt="netmatters client" />
                        </picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Busseys</h3>
                                <p class="clients__text">One of the UK's leading Ford dealerships.</p>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/crane_logo.webp"
                                alt="netmatters client" />
                        </picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Crane Garden Buildings</h3>
                                <p class="clients__text">
                                    Leading manufacturer and supplier of high-end garden rooms, summerhouses,
                                    workshops
                                    and sheds in the
                                    UK.
                                </p>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/black_swan_logo.webp"
                                alt="netmatters client" />
                        </picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Black Swan Care Group</h3>
                                <p class="clients__text">
                                    Black Swan Care Group own and manage 21 high-quality care and residential homes
                                    with
                                    a focus on
                                    putting the needs of their residents first.
                                </p>
                                <a class="clients__btn" href="#">view our case study <span><i
                                            class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/xupes_logo.webp"
                                alt="netmatters client" />
                        </picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Xupes</h3>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/beat_logo.webp"
                                alt="netmatters client" /></picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">BEAT</h3>
                                <p class="clients__text">The UK's eating disorder charity founded in 1989</p>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/survey_solutions_logo.webp"
                                alt="netmatters client" /></picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Survey Solutions</h3>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/girl_guides_anglia_logo.webp"
                                alt="netmatters client" /></picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Girl Guiding Anglia</h3>
                                <p class="clients__text">
                                    Girl Guiding Anglia is part of Girlguiding, the UK's leading charity for girls
                                    and
                                    young women in
                                    the UK.
                                </p>
                                <a class="clients__btn" href="#">view our case study <span><i
                                            class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/sweetzy_logo.webp"
                                alt="netmatters client" />
                        </picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Sweetzy</h3>
                                <p class="clients__text">Sweetzy are an online sweets retailer, based in Wymondham.
                                </p>
                                <a class="clients__btn" href="#">view our case study <span><i
                                            class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/howespercivallogo.webp"
                                alt="netmatters client" />
                        </picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Howes Percival</h3>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/girls_day_school_trust_logob.png"
                                alt="netmatters client" /></picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">GDST</h3>
                                <p class="clients__text">
                                    The<a href="#" class="clients__content-link"> Girls' Day School Trust (GDST)</a>
                                    is
                                    the UK's leading
                                    family of 25 independent girls' schools.
                                </p>
                                <a class="clients__btn" href="#">view our case study <span><i
                                            class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/busseys_logo.webp"
                                alt="netmatters client" />
                        </picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Busseys</h3>
                                <p class="clients__text">One of the UK's leading Ford dealerships.</p>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/crane_logo.webp"
                                alt="netmatters client" />
                        </picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Crane Garden Buildings</h3>
                                <p class="clients__text">
                                    Leading manufacturer and supplier of high-end garden rooms, summerhouses,
                                    workshops
                                    and sheds in the
                                    UK.
                                </p>
                            </div>
                        </div>
                    </li>
                    <li class="clients__item">
                        <picture><img class="clients__image" src="./img/clients/black_swan_logo.webp"
                                alt="netmatters client" />
                        </picture>
                        <div class="clients__content">
                            <div class="clients__content-container">
                                <h3 class="clients__heading">Black Swan Care Group</h3>
                                <p class="clients__text">
                                    Black Swan Care Group own and manage 21 high-quality care and residential homes
                                    with
                                    a focus on
                                    putting the needs of their residents first.
                                </p>
                                <a class="clients__btn" href="#">view our case study <span><i
                                            class="fa-solid fa-arrow-right fa-sm"></i></span></a>
                            </div>
                        </div>
                    </li>
                </ul>
            </div>

            <!-- Newsletter -->

            <div class="full-width-container full-width-container--bg-newsletter">
                <div class="inner-container newsletter">
                    <form action="#" class="newsletter-form">
                        <h2 class="newsletter-form__heading">Email Newsletter Sign-Up</h2>
                        <div class="newsletter-form__inputs">
                            <div class="newsletter-form__inputs-container">
                                <label class="newsletter-form__label" for="name">Your Name</label>
                                <input class="newsletter-form__input" type="text" name="newsletter-name" id="name" />
                            </div>
                            <div class="newsletter-form__inputs-container">
                                <label class="newsletter-form__label" for="email">Your Email</label>
                                <input class="newsletter-form__input" type="email" name="newsletter-email" id="email" />
                            </div>
                        </div>
                        <div class="newsletter-form__marketing-container">
                            <div class="newsletter-form__checkbox-container">
                                <input class="newsletter-form__checkbox--hidden" type="checkbox" name="marketing"
                                    id="marketing-checkbox-hidden" />
                                <label class="newsletter-form__checkbox" id="checkbox-label"
                                    for="marketing-checkbox-hidden"></label>
                            </div>

                            <label class="newsletter-form__marketing-text" for="marketing-checkbox-hidden">
                                Please tick this box if you wish to receive marketing information from us. Please
                                see
                                our
                                <a href="#" class="newsletter-form__privacy-link">Privacy Policy</a> for more
                                information on how we
                                keep your data safe.</label>
                        </div>
                        <button class="newsletter-form__btn" type="submit">Subscribe</button>
                    </form>
                </div>
            </div>

            <!-- Footer -->
            <?php include './includes/footer.php' ?>
        </div> <!-- end main-content-->
    </div> <!-- end main-container-->
    <script type="text/javascript" src="//code.jquery.com/jquery-1.11.0.min.js"></script>
    <script type="text/javascript" src="//code.jquery.com/jquery-migrate-1.2.1.min.js"></script>
    <script type="text/javascript" src="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js"></script>
    <script type="text/javascript" src="js/slick.js"></script>
</body>

</html>